from umath import cos, sin, atan2, pi, degrees, radians, sqrt, copysign, asin, fabs
...
class vec2:
    ...
    def split(self, oreintation: float):
        ...
        return vec2(self.length() * cos(angleDiff(self.xAngle() - oreintation, 0)), 0).rotated(oreintation)
...
class Line:
    def __init__(self, a: vec2, b: vec2, endless=True):
        self.a = a
        self.b = b
    ...
    def fPoint(self, point: vec2):
        ...
        return (self.a - point).split(self.orientation + pi/2)
    
    def intersection(self, other): #AI generated, not tested
        """returns intersection point of two lines, or None if they are parallel or not intersecting (if not endless)"""
        d = self.direction.x * other.direction.y - self.direction.y * other.direction.x
        if d == 0:
            return None # parallel lines
        t = ((other.a.x - self.a.x) * other.direction.y - (other.a.y - self.a.y) * other.direction.x) / d
        u = ((other.a.x - self.a.x) * self.direction.y - (other.a.y - self.a.y) * self.direction.x) / d
        if not self.endless and (t < 0 or t > 1):
            return None # not intersecting within the line segment
        if not other.endless and (u < 0 or u > 1):
            return None # not intersecting within the line segment
        return self.a + t * self.direction

class Area:
    def __init__(self, sides: list[Line]):
        self.sides = sides
